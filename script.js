...
const htmlCodeLines = [
    ...
    '   &lt;/<span class="dark-blue">h6</span>&gt;',
    '<span class="enter-line real-text h6-heading"></span>',
    '<span class="enter-line real-text h6-heading"></span>',
    '<span class="enter-line real-text h6-heading"></span>',
    '<span class="enter-line real-text h6-heading"></span>',
    '<span class="enter-line real-text h6-heading"></span>',
...
const javascriptCodeLines = [
    ...
    '<span class="real-text paragraph">Hi, I’m Devon Hill, a Computer Science graduate from Kansas State University with experience in software development, testing, and IT support. During my time as a Teaching Assistant, I supported students in debugging and strengthening their programming skills. As a Summer Support Assistant in IT, I gained hands-on experience maintaining and troubleshooting technology systems. I also contributed to CyberPipeline, a full-stack web application project, where my role focused on backend API testing, Git version control, and Agile practices. I’m passionate about problem-solving, continuous learning, and building tools that make processes more efficient. When I’m not coding, you can find me practicing Spanish, exploring new challenges, finding creative ways to grow personally and professionally.</span>',
    '   <span class="magenta">}</span>',
    '   <span class="pale-yellow">showTechStack</span><span class="magenta">() {</span>',
    '       <span class="dark-blue">const</span> <span class="sky-blue">techStack</span> <span class="white">=</span> <span class="dark-blue">[</span>',
    '<span id="logos">' + javaLogo + javascriptLogo + csharpLogo + sqlLogo + gitLogo + gitHubLogo + dockerLogo + '</span>',
    '       <span class="dark-blue">]</span><span class="white">;</span>',
    '       <span class="sky-blue">techStack</span><span class="white">.</span><span class="pale-yellow">forEach</span><span class="dark-blue">(</span><span class="light-blue">tech</span> <span class="dark-blue">=></span> <span class="light-blue">console</span><span class="white">.</span><span class="pale-yellow">log</span><span class="bright-yellow">(</span><span class="light-blue">tech</span><span class="bright-yellow">)</span><span class="dark-blue">)</span><span class="white">;</span>',
    '   <span class="magenta">}</span>',
...
htmlCode.innerHTML = htmlCodeLines.map(line => `<div>${line}</div>`).join('');

const updateLineNumbers = (typeOfCode) => {
    const divs = typeOfCode.children;
    let num = 1;
    Array.from(divs).forEach((div, index) => {
        const numberOfLines = Math.round(div.scrollHeight / parseFloat(getComputedStyle(lineNumbers).lineHeight));
...
const enterLines = document.querySelectorAll('.enter-line');
...
const typeWordAnimation = async (word, container) => {
 ...
 const deleteWordAnimation = async (word) => {
    return new Promise(async (resolve) => {
 ...
 const tagLineAnimation = async () => {
    await typeWordAnimation(firstWord, typingText);
    await sleep(500);
    await deleteWordAnimation(firstWord);
    await sleep(500);
    await typeWordAnimation(secondWord, typingText);
    await sleep(500);
    await deleteWordAnimation(secondWord);
    await sleep(500);
    await typeWordAnimation(thirdWord, typingText);
    endingIndex = thirdWord.length;
    await sleep(800);
    await typeWordAnimation(fourthWord, typingText);
    await sleep(500);
    await deleteWordAnimation(fourthWord);
    await sleep(500);
    await typeWordAnimation(fifthWord, typingText);
    await sleep(500);
    await deleteWordAnimation(fifthWord);
    await sleep(500);
    await typeWordAnimation(sixthWord, typingText);
    endingIndex += sixthWord.length;
    await sleep(800);
    await typeWordAnimation(seventhWord, typingText);
    await sleep(500);
    await deleteWordAnimation(seventhWord);
    await sleep(500);
    await typeWordAnimation(finalWord, typingText);
    await sleep(500);
    typingText.classList.add('blinking');
    await sleep(200);
 }

 const moveCursorDownAnimation = async () => {
    typingText.classList.remove('blinking');
    typingText.style.borderRight = 'none';
    for(const line of enterLines) {
        line.setAttribute('style', 'border-left: 2px solid #ffffff; height: 20px; width: 100%; display: block; line-height: 20px;');
...
const printAsciiArt = async () => {
    for(let i = 0; i < asciiCodeArtLines.length; i++) {
    await typeWordAnimation(asciiCodeArtLines[i], enterLines[i+2]);
    }
}
tagLineAnimation().then(() =>{
    moveCursorDownAnimation().then(() => {
